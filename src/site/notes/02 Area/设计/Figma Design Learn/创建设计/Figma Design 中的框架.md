...
注意：如果你已对任何子对象应用了[[02 Area/设计/Figma Design Learn/应用约束以定义图层如何调整大小\|约束]]，Figma 将调整它们的大小以匹配新的框架预设。否则，框架内的对象将保持原始尺寸和位置。
