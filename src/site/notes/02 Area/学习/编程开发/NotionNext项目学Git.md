---
{"dg-publish":true,"dg-permalink":"learn/program/git","permalink":"/learn/program/git/","metatags":{"description":"借用NotionNext项目实战操作Git，实现本地及远程版本控制管理","og:site_name":"DavonOs","og:title":"Git","og:type":"article","og:url":"https://zuji.eu.org/learn/program/git","og:image":null,"og:image:width":"400","og:image:alt":"articlecover","og:locale":"zh_cn"}}
---
...
在Github上找到您要同步的项目，并复制项目的URL。打开命令行工具（如Terminal或Command Prompt）。
...
使用 git fetch 命令来获取远程的分支信息。此命令会将远程分支的信息拉取到本地，但并不会自动地在本地创建这些分支。输入以下命令：`git fetch`查看远程分支列表。

您可以使用以下命令查看获取到的远程分支列表：`git branch -r`
...
使用 `git log`命令查看提交历史，查询到希望回滚的具体提交的哈希值或引用（如分支名）。

使用 `git reset` 命令指定要回滚的提交的哈希值或引用（如分支名）进行版本回滚：
...
这将删除项目目录中名为`.git`的Git版本控制文件夹，从而完全删除与Git相关的信息。
...
如果要为远程存储库的分支起一个不同名的分支，可以指定推送到的远程分支：`git push origin feature-branch:new-feature-branch`
