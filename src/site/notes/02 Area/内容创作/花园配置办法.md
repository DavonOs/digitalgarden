---
{"dg-publish":true,"dg-permalink":"digital-garden/config","permalink":"/digital-garden/config/","metatags":{"description":"这里是 🏡Davon的数字花园，是个人不断发展的想法的集合，作为半成品的思考，在可探索的空间中，随时间推移不断播种、修剪、塑造","og:site_name":"DavonOs","og:title":"关于","og:type":"article","og:url":"https://zuji.eu.org/digital-garden/config","og:image":null,"og:image:width":"400","og:image:alt":"articlecover","og:locale":"zh_cn"},"dgShowInlineTitle":true,"dg-note-properties":{"tags":null}}
---
...
Appearance → Manage appearance → theme→我的选择：obsidiannotion
...
```
添加到 userComputed 函数返回的对象中的任何属性，都可以在你的 `.njk` 文件中使用。例如，以下 userUtils.js 文件，让你的模板中提供城市和天气属性：
...
1. 未能成功渲染 Dataview 内联数据查询，[Issue Discussion](https://github.com/oleeskild/digitalgarden/issues/141)
2. 未支持 Dataview 的 Calendar 渲染
3. 未支持 Excalidraw 插件内的双链等其他功能，[Issue Discussion 132](https://github.com/oleeskild/obsidian-digital-garden/discussions/132)
...
5. 代码块内使用 html 代码：`a href="\"` 被误识别为反向链接到主页。
解决方案：需要在 `extractLinks` 方法中，先去除代码块内容（包括 markdown 代码块和 HTML `<pre>` 块），再提取链接。
...
### 功能更新
#### 不蒜子统计插入 DigitalGarden 完整教程

核心思路：在 DigitalGarden 全局模板引入不蒜子脚本，在页脚 / 文章页插入统计标签，用项目自带样式文件做美化，全程**不改核心配置、不破坏项目结构**，5 分钟完成。

一、先明确项目关键路径（基于 oleeskild/digitalgarden）

- 全局布局模板：`src/_includes/layouts/base.njk`（所有页面继承，插入脚本必选）
- 页脚组件：`src/_includes/partials/footer.njk`（放全站统计最优）
- 文章模板：`src/_includes/layouts/note.njk`（放单页阅读量）
- 自定义样式：`src/site/styles/custom-style.scss`（统一美化）

二、Step 1：全局引入不蒜子脚本（必做）

打开 `src/_includes/layouts/base.njk`，在 ** `</head>` 结束标签前** 粘贴以下脚本**二选一**：
...
## 三、Step 2：插入统计标签（选位置粘贴）

### 位置 1：全站页脚（推荐，显示全站 / 今日数据）

打开 `src/_includes/partials/footer.njk`，在版权行附近添加：
...
<div class="busuanzi-stats">
  ...
  今日访问 <span id="busuanzi_today_pv">加载中...</span> 次
</div>
```

### 位置 2：单篇文章底部（显示阅读量）

打开 `src/_includes/layouts/note.njk`，在文章内容结束后、页脚前添加：
...
```
<!-- 单页阅读量 -->
<div class="busuanzi-page">
  本文阅读量 <span id="busuanzi_page_pv">加载中...</span> 次
</div>
```

---

## 四、Step 3：自定义样式与加载动画（可选）

### 1. 样式美化（用项目自定义样式文件）

打开 `src/site/styles/custom-style.scss`，添加：

scss

```
/* 不蒜子统计样式 */
.busuanzi-stats, .busuanzi-page {
  color: var(--text-muted);
  font-size: 0.85rem;
  margin: 1rem 0;
  text-align: center;
}
#busuanzi_site_pv, #busuanzi_site_uv, #busuanzi_page_pv {
  margin: 0 4px;
  font-weight: 500;
}
```
...
```
<link rel="stylesheet" href="https://cdn.coolcdn.cn/static/font-awesome/7.0.0/css/all.min.css">
```
...
```
<span id="busuanzi_site_pv"><i class="fa fa-spinner fa-spin"></i></span>
```

---

## 五、Step 4：本地部署脚本（可选，不用 CDN）

1. 下载不蒜子 JS：[https://cdn.busuanzi.cc/download/Busuanzi_3.6.9.zip](https://cdn.busuanzi.cc/download/Busuanzi_3.6.9.zip)
2. 解压后将 `busuanzi.min.js` 放入 `src/site/scripts/`
3. 把脚本路径改为本地：
...
```
<script src="/site/scripts/busuanzi.min.js" defer></script>
```
...
1. 本地运行：`npm run dev`
2. 访问页面，查看统计数字是否正常加载
3. 构建部署：`npm run build`，发布后全站生效

---

## 七、核心说明

- PV：每次访问 + 1；UV：同一天同一 IP 只算 1 次
- 今日数据**每日 00:00 自动重置**
- 只统计不显示：仅引入脚本，不贴标签即可
