...
`common`：将在主页和所有笔记页面上添加
`index`：将在主页上添加
`notes`：将添加到所有笔记页面上

目前支持的插槽：

head：将被放置在 html 的 `<head>` 标签内部
header：将放置在页面顶部，在 `<header>` 标签中，如果启用了标签和注释标题，则放置在它们之后
beforeContent：将放置在页面内容之前
afterContent：将放置在页面内容之后
footer：将放置在页面底部
