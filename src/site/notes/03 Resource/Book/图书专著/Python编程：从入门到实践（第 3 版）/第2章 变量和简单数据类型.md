---
{"dg-publish":true,"dg-permalink":"python/变量和数据类型","permalink":"/python/变量和数据类型/","metatags":{"description":"Python编程：从入门到实践（第3版）第2章 变量和数据类型的学习笔记","og:site_name":"DavonOs","og:title":"第1章 起步","og:type":"article","og:url":"https://zuji.eu.org/python/变量和数据类型","og:image":"https://weread-1258476243.file.myqcloud.com/weread/cover/90/YueWen_34336681/t6_YueWen_34336681.jpg","og:image:width":"400","og:image:alt":"articlecover","og:locale":"zh_cn"},"tags":["digitalgarden"]}
---
...
使用空白来显示整洁的输出，如何删除字符串中的多余字符。
如何使用整数和浮点数，了解使用数值数据的方式。
如何编写说明性注释，了解 Python 代码编写的理念。
## 2.1 运行hello_world.py时的情况

Python 解释器：读取整个程序，确定其中每个单词的含义并执行。

例如，当解释器看到 `print`，就会将括号中的内容打印到屏幕上。你可以把 Python 解释器当作一个黑盒子，只要知道它大概的作用就行。
...
用in或者not in这两种方法均可实现。除了in和not in，还可以用find，当用find查找某一字符是否存在于某个字符串中时，如果存在则返回该字符的具体位置，如果不存在则返回-1，如下所示。
...
input() 函数让程序暂停运行，等待用户输⼊⼀些⽂本。获取用户输⼊ 后，Python 将其赋给⼀个变量，以便使用。 例如，下面的程序让用户输⼊⼀些⽂本，再将这些⽂本呈现给用户：
...
input() 函数接受⼀个参数，即要向用户显示的提示（prompt），让用户知道该输⼊什么样的信息。在这个示例中，当 Python 运行第⼀行代码时， 用户将看到提示“Tell me something, and I will repeat it back to you:”。程序等待用户输⼊，并在用户按回⻋键后继续运行。用户的输⼊被赋给变量 message，接下来的 print(message) 将输⼊呈现给用户。

注意：有些⽂本编辑器不能运行提示用户输⼊的程序。你可使用这些⽂本编辑器编写提示用户输⼊的程序，但必须从终端运行它们。

7.1.1 编写清晰的提示

每当使用 input() 函数时，都应指定清晰易懂的提示，准确地指出希望用 户提供什么样的信息——能指出用户应该输⼊什么信息的任何提示都行， 如下所示：
...
通过在提示末尾（这里是冒号后面）添加⼀个空格，可将提示与用户输⼊分开，让用户清楚地知道其输⼊始于何处。
...
这个示例演示了⼀种创建多行字符串的方式。第⼀行将消息的前半部分赋给变量 prompt。在第二行中，运算符 += 在赋给变量 prompt 的字符串末尾追加⼀个字符串。最终的提示占两行，且问号后面有⼀个空格，这也是为了使其更加清晰。

7.1.2 使用 int() 来获取数值输⼊

在使用 input() 函数时，Python 会将用户输⼊解读为字符串。请看下面让 用户输⼊年龄的解释器会话：
...
用户输⼊的是数 21，但当我们请求 Python 提供变量 age 的值时，它返回的是 '21'——用户输⼊数值的字符串表示。我们怎么知道 Python 将输⼊解读成了字符串呢？因为这个数是用引号引起来的。如果只想打印输⼊，这⼀点⼉问题都没有；但如果试图将输⼊作为数来使用，就会引发错误：
...
当试图将该输⼊用于数值比较时，Python 会报错，因为它无法将字符串和整数进行比较：不能将赋给 age 的字符串'21'与数值18进行比较。为了解决这个问题，可使用函数 int()将输⼊的字符串转换为数值，确保能够成功地执行比较操作：
...
在这个示例中，当用户根据提示输⼊21后，Python 将这个数解读为字符串，但随后int()将这个字符串转换成了数值表示。这样 Python 就能运行条件测试了：将变量 age（它现在表示的是数值21）同18进行比较，看它是否大于或等于18。测试结果为 True。

如何在实际程序中使用 int()函数呢？请看下面的程序，它判断⼀个人是否满足坐过⼭⻋的⾝高要求1：
...
在这个程序中，为何可以将height与48进行比较呢？因为在比较前，height = int(height)将输⼊转换成了数值表示。如果输⼊的数大于或等于48，就指出用户满足⾝高条件。

在将数值输⼊用于计算和比较前，务必将其转换为数值表示。
...
如果⼀个数可被另⼀个数整除，余数就为0，因此求模运算将返回0。可利用这⼀点来判断⼀个数是奇数还是偶数：
