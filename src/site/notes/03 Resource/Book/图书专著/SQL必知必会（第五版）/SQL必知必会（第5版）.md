...
SQL（发音为字母 S-Q-L 或 sequel），结构化查询语言（Structured Query Language）的缩写，是一种专门用来与数据库沟通的语言。
