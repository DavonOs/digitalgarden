...
构建网格布局时会涉及到这些[组成部分](https://so.csdn.net/so/search?q=%E7%BB%84%E6%88%90%E9%83%A8%E5%88%86\&spm=1001.2101.3001.7020)。例如，声明 `grid-template-columns: 1fr 1fr 1fr` 就能定义三个等宽且垂直的 **网格轨道**，同时还定义了四条垂直的的 **网格线**：一条位于网格的最左边；另外两条位于每个网格轨道之间，还有一条则位于网格的最右边。
